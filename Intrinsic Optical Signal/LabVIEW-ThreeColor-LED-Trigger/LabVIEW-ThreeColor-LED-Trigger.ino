...
int blueLEDs = 3;            // blue LEDs output pin (4 T-Cube LED drivers attached to arduino uno
int redLEDs = 4;             // red LEDs output pin (4 T-Cube LED drivers attached to arduino uno
int greenLEDs = 5;           // green LEDs output pin (4 T-Cube LED drivers attached to arduino uno
...
void loop()
{
  ...
  if (trialState == HIGH)
  {
    ...
    if (prevTrialState == 0)
    {
      prevBlueState = 0;                        // Ensure that the first flash of every new trial is from green LEDs
      prevRedState = 0;
      prevGreenState = 1;
    }
    prevTrialState = 1;
  ...
  if (counterState != prevCounterState)         // Checks if the counter state that was just read is different from previous
  {
    if (counterState == HIGH)                   
    {
      if (prevGreenState == 1) 
      {
        ...
        prevRedState = 0;                      // Conforms the state variable of red LEDs to what its state is (red LEDs is LOW, so state variable changed to 0)
      ...
      else if (prevBlueState == 1)
      {
        ...
        prevRedState = 1;                      // Conforms the state variable of red LEDs to what its state is (red LEDs is HIGH, so state variable changed to 1)
        prevGreenState = 0;                    // Conforms the state variable of green LEDs to what its state is (green LEDs is LOW, so state variable changed to 0)
      }      
       else if (prevRedState == 1)
      {
        if (trialState == HIGH)
        {
          digitalWrite(IOScam,HIGH); 
        ...
        prevBlueState = 0;                     // Conforms the state variable of blue LEDs to what its state is (blue LEDs is LOW, so state variable changed to 0)
        prevRedState = 0;                      // Conforms the state variable of red LEDs to what its state is (red LEDs is LOW, so state variable changed to 0)
        prevGreenState = 1;                    // Conforms the state variable of green LEDs to what its state is (green LEDs is HIGH, so state variable changed to 1)
      }      
    }
    else
    {
